  ...
  return (
    <div className="flex h-screen w-screen flex-col items-center justify-center gap-8 bg-background px-8">
      ...
      <div className="flex flex-col items-center gap-4 text-center">
        <div className="flex h-16 w-16 items-center justify-center rounded-xl bg-surface-container-highest">
      ...
      <div className="flex gap-3">
        {Array.from({ length: MAX }).map((_, i) => (
          <div
            key={i}
            className={`flex h-20 w-14 items-center justify-center rounded-xl font-mono text-3xl font-bold transition-all ${
              i < digits.length
                ? "bg-primary-container text-primary-container-foreground"
                : i === digits.length
                  ? "bg-surface-bright animate-pulse text-foreground"
      ...
      <Button
        ...
        className="h-14 w-full max-w-xs rounded-xl text-lg font-semibold disabled:opacity-30"
      >
        დადასტურება
      </Button>
    </div>
